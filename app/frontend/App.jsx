...
function EmployeesPanel() {
  ...
  const pages = useMemo(() => Math.max(1, Math.ceil(total / per)), [total, per])
  ...
  return (
    <div className="space-y-4">
        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-3 py-2 text-left font-medium text-slate-700">Country</th>
              <th className="px-3 py-2 text-right font-medium text-slate-700">Salary</th>
            ...
            {items.map((row) => (
              <tr key={row.id} className="hover:bg-slate-50">
                ...
                <td className="px-3 py-2 text-right space-x-2">
                  ...
                  <button type="button" className="text-red-600 hover:underline" onClick={() => remove(row.id)}>
                    Delete
                  </button>
                </td>
              </tr>
      ...
      <div className="flex items-center justify-between text-sm text-slate-600">
        <span>
          Page {page} of {pages} — {total.toLocaleString()} employees
        </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white p-6 shadow-xl">
            ...
            <form className="mt-4 space-y-3" onSubmit={submit}>
              ...
              <div>
                <label className="text-xs font-medium text-slate-600">Annual salary</label>
                <input
                  required
                  type="number"
                  min="1"
              ...
              <div>
                ...
                <input
                  ...
                  title={modal?.mode === "edit" ? "Email is the stable identifier in this demo" : ""}
                />
              </div>
              <div>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="min-w-[240px] flex-1">
          <label className="block text-xs font-medium text-slate-600">Job title filter (optional)</label>
          <input
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
      ...
      {data ? (
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <h3 className="text-sm font-semibold text-slate-800">Country-wide salary</h3>
            <dl className="mt-3 space-y-2 text-sm">
              <div className="flex justify-between">
                <dt className="text-slate-600">Employees</dt>
                <dd className="font-medium tabular-nums">{data.overall?.employee_count}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-600">Minimum</dt>
                <dd className="font-medium tabular-nums">{data.overall?.min_salary?.toLocaleString?.()}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-600">Maximum</dt>
                <dd className="font-medium tabular-nums">{data.overall?.max_salary?.toLocaleString?.()}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-600">Average</dt>
                <dd className="font-medium tabular-nums">{data.overall?.avg_salary?.toLocaleString?.()}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-600">Total payroll</dt>
                <dd className="font-medium tabular-nums">{data.overall?.total_payroll?.toLocaleString?.()}</dd>
              </div>
            </dl>
          </div>
          {data.filtered_by_job_title ? (
            <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <h3 className="text-sm font-semibold text-slate-800">Selected job title</h3>
              <p className="mt-1 text-xs text-slate-500">Matches exact title within the country.</p>
              <dl className="mt-3 space-y-2 text-sm">
                <div className="flex justify-between">
                  <dt className="text-slate-600">Employees</dt>
                  <dd className="font-medium tabular-nums">{data.filtered_by_job_title.employee_count}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-slate-600">Average</dt>
          ...
          <div className="md:col-span-2 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <h3 className="text-sm font-semibold text-slate-800">Top job titles (headcount & averages)</h3>
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {(data.job_title_breakdown || []).map((row) => (
                    <tr key={row.job_title} className="border-b border-slate-100">
                      <td className="py-2 pr-4 font-medium text-slate-900">{row.job_title}</td>
                      <td className="py-2 pr-4 tabular-nums">{row.employee_count}</td>
                      <td className="py-2 pr-4 tabular-nums">{row.avg_salary?.toLocaleString?.()}</td>
                      <td className="py-2 pr-4 tabular-nums">{row.min_salary?.toLocaleString?.()}</td>
                      <td className="py-2 tabular-nums">{row.max_salary?.toLocaleString?.()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
